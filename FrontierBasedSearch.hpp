/* 
 * A modified version of tdzdd/spec/FrontierBasedSearch.hpp for Capacitated Arc Routing Problem
 * Added Count update to reset Count after every graph copy
...
class FrontierBasedSearch: public tdzdd::HybridDdSpec<FrontierBasedSearch,
    ...
    int takable(Count& c, Mate const* mate, Graph::EdgeInfo const& e) const {
        ...
        if (e.finalEdge && c.uec > 0 && !nullPath) return false; //TODO 枝刈り可能
        if (e.finalEdge && c.uec > 0 && c.uec != numUEC && nullPath) return false; // CARP: Null paths
        return true;
    }
    ...
    void update(Count& count, Mate* mate, Graph::EdgeInfo const& e,
        ...
        // CARP: Reset count after every copy of graph
        if (e.finalEdge) {
            count = Count(numUEC);
        }
...
public:
    FrontierBasedSearch(Graph const& graph, int numUEC = 1, bool nullPath = false,
            ...
            : graph(graph), m(graph.vertexSize()), n(graph.edgeSize()),
              ...
              numUEC(numUEC), noLoop(noLoop), nullPath(nullPath), lookahead(lookahead) {
