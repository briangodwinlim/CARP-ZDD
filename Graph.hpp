/* 
 * A modified version of tdzdd/util/Graph.hpp for Capacitated Arc Routing Problem
 * Added duplicateGraph(copy) to duplicate graph copy times
 * Added numCopyGraph() to get number of copies of graph
 * Added numEdgesGraph() to get number of edges per graph copy
 * Added numRequiredEdges() to get number of required edges with nonzero demand
 * Added getEdgeCost() and getEdgeDemand() to get edge cost and demand
 * Modified update() to optimize frontier size
 * Modified addEdge(v1, v2, cost, demand) to include cost and demand information
 */
...
private:
    ...
    bool hasColorPairs_;
    // CARP: Cost and demand per edge
    std::map<std::pair<std::string,std::string>,int> edge2cost;
    std::map<std::pair<std::string,std::string>,int> edge2demand;
    // CARP: Number of required edges
    int numRequiredEdges_ = 0;

public:
    // CARP: Add index to vertex and cost/demand information per edge
    void addEdge(std::string vertexName1, std::string vertexName2, int cost = 0, int demand = 0) {
        edgeNames.push_back(std::make_pair(vertexName1 + "_0", vertexName2 + "_0"));
        edge2cost[std::make_pair(vertexName1 + "_0", vertexName2 + "_0")] = cost;
        edge2demand[std::make_pair(vertexName1 + "_0", vertexName2 + "_0")] = demand;
        if (demand > 0) ++numRequiredEdges_;
        ++numEdgesOne_;
    }
...
public:
    ...
    void update() {
        ...
        for (size_t i = 0; i < edgeNames.size(); ++i) edge2name.push_back(edgeNames[i]);
        
        // Sort vertices by leaving order
        {
            vertex2name.push_back(""); // begin vertex number with 1
            
            // CARP: Change order for optimzed frontier size
            for (int c = 0; c < copies; ++c) {
                for (size_t i = numEdgesOne_ * (c + 1) - 1; i + 1 > numEdgesOne_ * c; --i) {
                    std::string const& s1 = edge2name[i].first;
                    std::string const& s2 = edge2name[i].second;

                    if (name2vertex.count(s2) == 0) {
                        name2vertex[s2] = vertex2name.size();
                        vertex2name.push_back(s2);
                    }

                    if (name2vertex.count(s1) == 0) {
                        name2vertex[s1] = vertex2name.size();
                        vertex2name.push_back(s1);
                    }
                    
                    if (vertex2name.size() > size_t(MAX_VERTICES))
                        throw std::runtime_error("ERROR: Vertex number > " + to_string(MAX_VERTICES));
                }
            }
        }
...

        {
            ...
            for (EdgeNumber a = 0; a < edgeSize(); ++a) {
                ...
                // CARP: Final edge for each copy of graph
                e.finalEdge = (a % numEdgesOne_ == numEdgesOne_ - 1);
            }
        }
    }
...
public:
    // CARP: Duplicate graph (optimized for frontier size)
    void duplicateGraph(int copy) {
        if (copies != 1) throw std::runtime_error("ERROR: Graph already duplicated");

        copies = copy;
        for (int c = 1; c < copies; ++c) {
            for (int i = 0; i < numEdgesOne_; ++i) {
                std::string u = edgeNames[i].first;
                std::string v = edgeNames[i].second;
                u = u.substr(0, u.length() - 1);
                v = v.substr(0, v.length() - 1);
                edgeNames.push_back(std::make_pair(u + to_string(c), v + to_string(c)));
                edge2cost[std::make_pair(u + to_string(c), v + to_string(c))] = edge2cost[std::make_pair(u + "0", v + "0")];
                edge2demand[std::make_pair(u + to_string(c), v + to_string(c))] = edge2demand[std::make_pair(u + "0", v + "0")];
                if (edge2demand[std::make_pair(u + to_string(c), v + to_string(c))] > 0) ++numRequiredEdges_;
            }
        }

        update();

        // Check if edges are properly ordered
        for (int i = 0; i < numEdgesOne_; ++i) {
            std::string u = edgeNames[i].first;
            std::string v = edgeNames[i].second;
            u = u.substr(0, u.length() - 1);
            v = v.substr(0, v.length() - 1);
            for (int c = 1; c < copies; ++c) {
                std::string uu = edgeNames[i + numEdgesOne_ * c].first;
                std::string vv = edgeNames[i + numEdgesOne_ * c].second;
                uu = uu.substr(0, u.length());
                vv = vv.substr(0, v.length());
                assert(uu == u && vv == v);
            }
        }
    }

    // CARP: Get number of copies of graph
    int numCopyGraph() const {
        return copies;
    }

    // CARP: Get number of edges per graph
    int numEdgesGraph() const {
        return numEdgesOne_;
    }

    // CARP: Get number of required edges
    int numRequiredEdges() const {
        return numRequiredEdges_;
    }

    // CARP: Get edge cost
    int getEdgeCost(EdgeNumber e) const {
        return edge2cost.at(edgeName(e));
    }

    int getEdgeCost(std::string vertexName1, std::string vertexName2) const {
        return edge2cost.at(std::make_pair(vertexName1, vertexName2));
    }

    // CARP: Get edge demand
    int getEdgeDemand(EdgeNumber e) const {
        return edge2demand.at(edgeName(e));
    }

    int getEdgeDemand(std::string vertexName1, std::string vertexName2) const {
        return edge2demand.at(std::make_pair(vertexName1, vertexName2));
    }
};
